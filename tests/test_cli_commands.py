...
#
# There seems to be a bug only present on Windows where
# the test for deleting the database file fails.
...
def test_crawl():
    ...
    runner = app.test_cli_runner()
    result = runner.invoke(crawl_command, ['--threads', threads, '--pages', pages])
...
def test_del_db():
    runner = app.test_cli_runner()
    runner.invoke(del_command, ['-y'])

    result = os.path.exists(os.path.join(PROJECT_ROOT, DB_FILE))
